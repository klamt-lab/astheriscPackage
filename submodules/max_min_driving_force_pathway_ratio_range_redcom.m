...
function [mmdf, fv, x, min_df, max_df, dfs, fva_lb, fva_ub, reac_map, obj, z_vars, flux_vars, c_vars, first_z, first_fv, deltaGf]= max_min_driving_force_pathway_ratio_range_redcom(maximalMilpRunTime, reactionSpeciesIndexMapping, stoichMat, st, reacMin,...
...
%%% <PSB START
number_species = max(reactionSpeciesIndexMapping);
f_vars = obj.cpx.numVarArray(number_species, 0, 1.0);
obj.cpx.add(obj.cpx.eq(obj.cpx.sum(f_vars), 1.0));

for currentSpecies = 1:number_species
    % Get all reactions of current species
    species_reaction_indices = [];
    for currentReactionIndex = 1:length(reactionSpeciesIndexMapping)
        if reactionSpeciesIndexMapping(currentReactionIndex) == currentSpecies
            species_reaction_indices = [species_reaction_indices currentReactionIndex];
        end
    end

    [~, species_reaction_indices]= ismember([species_reaction_indices, -species_reaction_indices], reac_map);
    species_reaction_indices(species_reaction_indices==0) = [];
    for currentSpeciesReactionIndex = species_reaction_indices
        obj.cpx.add(obj.cpx.le(flux_vars(currentSpeciesReactionIndex), obj.cpx.prod(f_vars(currentSpecies), flux_ub(currentSpeciesReactionIndex))));
    end
end
%%% PSB END>
