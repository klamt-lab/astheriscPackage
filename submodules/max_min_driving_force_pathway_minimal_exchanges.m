function [mmdf, fv, x, min_df, max_df, dfs, fva_lb, fva_ub, reac_map, number_of_extra_exchanges, obj, z_vars, flux_vars, c_vars, first_z, first_fv, deltaGf]= max_min_driving_force_pathway_minimal_exchanges(maximalMilpRunTime, minimal_mmdf, exchange_reaction_indices,...
  maximal_number_exchange_reactions,...
  stoichMat, st, reacMin,...
...
%%% PSB and AvK change START ->
% Minimal mmdf constraint
obj.cpx.addGe(mmdf_var, minimal_mmdf);

[~, exchange_reaction_indices]= ismember([exchange_reaction_indices, -exchange_reaction_indices], reac_map);
if maximal_number_exchange_reactions == -1
    obj.cpx.addMinimize(obj.cpx.sum(z_vars(exchange_reaction_indices)));
else
    obj.cpx.add(obj.cpx.le(obj.cpx.sum(z_vars(exchange_reaction_indices)), maximal_number_exchange_reactions));
end
%%% PSB and AvK change END <-
...
% PSB change
if maximal_number_exchange_reactions == -1
    if obj.cpx.getStatus().equals(cplex_inner.Status.Unknown) || obj.cpx.getStatus().equals(cplex_inner.Status.Infeasible)
        number_of_extra_exchanges = -1;
    else
        try
            number_of_extra_exchanges = obj.cpx.getObjValue();
        catch
            number_of_extra_exchanges = -1;
        end
    end
else
    number_of_extra_exchanges = NaN;
end
