function [mmdf, fv, x, min_df, max_df, dfs, fva_lb, fva_ub, reac_map, obj, z_vars, flux_vars, c_vars, first_z, first_fv, deltaGf, mmdf_var]= max_min_driving_force_pathway_higher_mdf_max_exchanges(maximalMilpRunTime, numMaxExchanges, exchange_reaction_indices, minimal_mdf, minimize_flux_sum, stoichMat, st, reacMin,...
...
epsilon = 10e-7; % Will be added to last_result
...
% PSB/AvK: Major change, as well as commenting out the setting of objectives
obj.cpx.addGe(mmdf_var, minimal_mdf);

if minimize_flux_sum
    obj.cpx.addMinimize(obj.cpx.sum(flux_vars));
else
    obj.cpx.addMinimize([]);
end
% obj.cpx.getObjective().setExpr([]);
%
